...
export function safeParseSettings(settings: string | number | true | { [key: string]: Json; } | Json[] | null): Record<string, any> {
  if (!settings) {
    return {};
  }
...
export function getSettingsProp<T>(
  settings: string | number | true | { [key: string]: Json; } | Json[] | null, 
  prop: string, 
  defaultValue: T
): T {
