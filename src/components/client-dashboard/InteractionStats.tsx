...
export const InteractionStats: React.FC<InteractionStatsProps> = ({ stats }) => {
  ...
  return (
    <>
      <Card className="transition-all hover:shadow-md border-l-4 border-blue-500">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Total Interactions</CardTitle>
          <MessageSquare className="h-4 w-4 text-blue-500" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold text-gray-900">{total_interactions}</div>
      ...
      <Card className="transition-all hover:shadow-md border-l-4 border-green-500">
        ...
        <CardContent>
          <div className="text-2xl font-bold text-gray-900">{active_days}</div>
          <p className="text-xs text-gray-500 mt-1">Days with AI interactions</p>
        </CardContent>
      </Card>
      
      <Card className="transition-all hover:shadow-md border-l-4 border-amber-500">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Avg. Response Time</CardTitle>
          <Clock className="h-4 w-4 text-amber-500" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold text-gray-900">{average_response_time}s</div>
          <p className="text-xs text-gray-500 mt-1">Average AI response time</p>
        </CardContent>
      </Card>
      
      <Card className="transition-all hover:shadow-md border-l-4 border-purple-500">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Top Query Types</CardTitle>
          <BarChart3 className="h-4 w-4 text-purple-500" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold text-gray-900">{top_queries.length}</div>
