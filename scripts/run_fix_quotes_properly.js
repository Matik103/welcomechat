...
console.log('Running migration to fix quotes in client data...');

try {
  // Path to the migration file
  const migrationFile = path.join(__dirname, '..', 'supabase', 'migrations', '20240930_fix_all_quotes_properly.sql');
  ...
  console.log('Executing migration to fix quotes in client data...');
  execSync(`psql "${dbUrl}" -f "${migrationFile}"`, { stdio: 'inherit' });
  
  console.log('\n✅ Migration completed successfully! Client data has been fixed to properly handle quotes.');
  console.log('✅ The create_new_client function has been updated to sanitize agent names and descriptions.');
